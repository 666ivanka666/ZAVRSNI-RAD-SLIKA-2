...
def init_posude(): # Kreiraj 4 posude
    for x in range(4):
        p = PyFloraPosuda(x, f"Posuda_{x + 1}")
        lista_posuda.append(p)

def init_biljke(): # Kreiraj 2 biljke
    for x in range(2):
        b = Biljka(x, f"Biljka {x + 1}", lista_posuda[x], f"slika_{x + 1}.jpg")
        b.dodaj_karakteristiku(preporuke)
        lista_biljaka.append(b)

def print_posude():
    for posuda in lista_posuda:
        posuda_iter = iter(posuda)
        iter_value = next(posuda_iter)
        naziv_posude = tk.LabelFrame (root, width=50, height=50, bg='#282828', fg='white',font=('Arial', 10)) #Frame
        posuda.limit = len(lista_posuda)
        naziv_posude.grid(column=0, row=0, columnspan=15, padx=15, pady=15)
        
        if iter_value == 0:
            naziv_posude.place(anchor="nw", x = 80, y = 15)
        if iter_value == 1:
            naziv_posude.place(anchor="nw", x = 190, y = 15)
        if iter_value == 2:
            naziv_posude.place(anchor="nw", x = 80, y = 90)
        if iter_value == 3:
            naziv_posude.place(anchor="nw", x = 190, y = 90)

        posuda_label = tk.Label(naziv_posude, text=f'{posuda.name}', bg='#282828', fg='white',font=('Arial', 10))
        posuda_label.grid(column=0, row=0, columnspan=15, padx=15, pady=15)
...
sync_image = tk.Button(root, text='Sync' ,bg='#282828', fg='white',font=('Arial', 10), command=lambda: sync_clicked())
...
# my_button = tk.Button(vrijednost_senzora, text='Dodati vode',bg='#282828', fg='white',font=('Arial', 10), command=lambda: clicked(voda.get()))
# my_button.pack()
...
#dodaj novu posudu (treba je bolje smjestit)


button_dodaj = tk.Button(root, text='Dodaj novu posudu', 
   width=20,command = lambda:upload_file())
#button_dodaj.grid(row=20,column=20,columnspan=4)
button_dodaj.grid(column=0, row=0, columnspan=15, padx=15, pady=15)
button_dodaj.place(anchor='sw', relx=0.5, rely=0.5)
